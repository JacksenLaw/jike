package com.jackson.common.extention;
...
public abstract class AbsPagedListAdapter<T, VH extends RecyclerView.ViewHolder> extends PagedListAdapter<T, VH> {
    ...
    public void onViewAttachedToWindow(@NonNull RecyclerView.ViewHolder holder) {
        if (!isHeaderPosition(holder.getBindingAdapterPosition()) && !isFooterPosition(holder.getBindingAdapterPosition())) {
            this.onViewAttachedToWindow2((VH) holder);
        }
    }

    /**
     * 添加视频item到PageListPlayDetector中并自动播放
     */
    ...
    /**
     * 从PageListPlayDetector中移除此视频item并自动停止
     */
