...
import com.jackson.network.schedulers.IoScheduler;
...
public abstract class Request<T, R extends Request> implements Cloneable {
    ...
    public Executor mExecutor = Scheduler.ioExecutor();
    ...
    public R observerOn(Scheduler scheduler) {
        mExecutor = scheduler.getExecutor();
        return (R) this;
    }
    ...
    /**
     * 异步请求 需要指定回调线程请使用：.observerOn(Scheduler.mainThread()) or .observerOn(Scheduler.ioThread())
     * @param callback
     */
        ...
        if (mCacheStrategy != CACHE_ONLY) {
            getCall().enqueue(new Callback() {
                ...
                public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                    final ApiResponse<T> result = parseResponse(response, callback);
                    mExecutor.execute(new Runnable() {
                        @Override
                        public void run() {
                            if (!result.success) {
