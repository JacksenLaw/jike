 ...
 * FileName: FixFragmentNavigator
 * Author: Luo
 * Date: 2020/3/17 21:01
 * Description: 定制的navigator，替换fragment的replace，改为hide、show
 */
...
public class FixFragmentNavigator extends FragmentNavigator {
    ...
    public NavDestination navigate(@NonNull Destination destination, @Nullable Bundle args, @Nullable NavOptions navOptions, @Nullable Navigator.Extras navigatorExtras) {
        if (mManager.isStateSaved()) {
            KLog.i("Ignoring navigate() call: FragmentManager has already"
                    + " saved its state");
            return null;
        }
        ...
        KLog.i("fragment id = " + destination.getId());
        frag = mManager.findFragmentByTag(tag);
        KLog.i("fragment = " + frag);
        if (frag != null) {
            ft.show(frag);
        } else {
