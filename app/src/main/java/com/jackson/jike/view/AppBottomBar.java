 ...
 * FileName: AppBottomBar
 * Author: Luo
 * Date: 2020/3/12 20:49
 * Description: 自定义view承载底部导航栏
 */
public class AppBottomBar extends BottomNavigationView {
    ...
    public AppBottomBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        List<BottomBar.TabsBean> tabs = config.tabs;
        ...
        for (BottomBar.TabsBean tab : tabs) {
            ...
            menuItem.setIcon(mIcons[tab.index]);
            KLog.i("tab.index = " + tab.index);
        }

        //此处给按钮icon设置大小
        int index = 0;
        for (BottomBar.TabsBean tab : config.tabs) {
            if (!tab.enable) {
                continue;
            }
        ...
        if (config.selectTab != 0) {
            BottomBar.TabsBean selectTab = config.tabs.get(config.selectTab);
            if (selectTab.enable) {
