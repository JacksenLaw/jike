...
public class RecordView extends View implements View.OnLongClickListener, View.OnClickListener {
    ...
    public RecordView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        fillColor = typedArray.getColor(R.styleable.RecordView_background_color, Color.WHITE);
        maxDuration = typedArray.getInteger(R.styleable.RecordView_maxDuration, 10);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isRecording) {
             ...
             * startAngle：开始的角度
             * sweepAngle：扫过的角度
             * useCenter：绘制圆弧时需不需要和中心相连接 ， 链接即变成扇形，不连接即是圆弧
             */
